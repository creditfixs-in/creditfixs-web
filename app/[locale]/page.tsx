    ...
    <>
      ...
      <section className="relative overflow-hidden bg-navy-950 text-white">
        ...
        <div aria-hidden="true" className="pointer-events-none absolute inset-0">
          <div className="absolute -top-40 left-1/2 h-[500px] w-[800px] -translate-x-1/2 rounded-full bg-emerald-500/15 blur-3xl" />
        ...
        <div className="relative mx-auto max-w-7xl px-4 pb-16 pt-14 sm:px-6 sm:pt-20 lg:px-8 lg:pb-24">
          <div className="grid items-center gap-12 lg:grid-cols-2">
            <div>
              <p className="animate-rise inline-flex items-center gap-2 rounded-full border border-emerald-400/30 bg-emerald-400/10 px-4 py-1.5 text-xs font-semibold uppercase tracking-wide text-emerald-300 sm:text-sm">
              ...
              <p className="animate-rise-delay-2 mt-6 max-w-xl text-base leading-relaxed text-slate-300 sm:text-lg">
                {dict.hero.subtitle}
              </p>
              <div className="animate-rise-delay-2 mt-8 flex flex-col gap-3 sm:flex-row">
                <Link
                  href={`${base}/contact`}
                  className="rounded-full bg-emerald-500 px-7 py-3.5 text-center text-base font-semibold text-navy-950 shadow-lg shadow-emerald-500/25 transition hover:bg-emerald-400"
                >
                  {dict.hero.cta1}
                </Link>
                <Link
                  href={`${base}/services`}
                  className="rounded-full border border-white/25 px-7 py-3.5 text-center text-base font-semibold text-white transition hover:border-emerald-400 hover:text-emerald-300"
                >
                  {dict.hero.cta2}
                </Link>
              </div>
            ...
            <div className="mx-auto w-full max-w-md rounded-3xl border border-white/10 bg-white/5 p-8 backdrop-blur-sm sm:p-10">
          ...
          <dl className="mt-14 grid grid-cols-2 gap-6 border-t border-white/10 pt-10 sm:grid-cols-4">
            {dict.hero.stats.map((s) => (
              <div key={s.label} className="text-center sm:text-left">
                <dt className="order-2 text-xs font-medium uppercase tracking-wide text-slate-400 sm:text-sm">
                  {s.label}
                </dt>
                <dd className="order-1 text-3xl font-extrabold text-emerald-400 sm:text-4xl">
                  {s.value}
                </dd>
              </div>
            ))}
            ...
            {serviceSlugs.map((slug) => {
              ...
              return (
                <li key={slug}>
                  ...
                  >
                    <span className="flex h-12 w-12 items-center justify-center rounded-xl bg-emerald-50 text-emerald-600 transition group-hover:bg-emerald-600 group-hover:text-white">
          ...
          {dict.process.steps.map((step, i) => (
            <li key={step.title} className="relative">
              <div className="flex items-center gap-4">
                <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-navy-900 text-lg font-extrabold text-emerald-400">
                  {i + 1}
                </span>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <ul className="mt-12 grid gap-5 sm:grid-cols-2">
            {dict.why.items.map((item) => (
              <li key={item.title} className="rounded-2xl border border-white/10 bg-white/5 p-6 sm:p-8">
      ...
      <section className="mx-auto max-w-7xl px-4 pb-16 sm:px-6 sm:pb-24 lg:px-8">
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-emerald-600 to-teal-700 px-6 py-14 text-center text-white sm:px-12 sm:py-16">
          ...
          <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <Link
              href={`${base}/contact`}
              className="w-full rounded-full bg-white px-8 py-3.5 text-base font-bold text-emerald-700 shadow-lg transition hover:bg-emerald-50 sm:w-auto"
            >
              {dict.cta.button}
            </Link>
            <a
